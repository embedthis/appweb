...
static void initialize(int argc, char **argv)
{
    ...
    if (!app->config) {
        app->config =
            mprParseJson(sfmt(
                             "{ name: '%s', title: '%s', description: '%s', version: '%s', \
            http: {}, esp: {}}",
...
static MprList *getRoutes()
{
    ...
    for (next = 0; (route = mprGetNextItem(app->host->routes, &next)) != 0; ) {
        ...
        if (filterRoutePattern) {
            mprLog("", 6, "Check route name \"%s\", prefix \"%s\" with \"%s\"", route->pattern, route->startWith,
                   filterRoutePattern);
        ...
        if (parent && parent->eroute &&
            ((EspRoute*) parent->eroute)->compileCmd &&
            smatch(route->documents, parent->documents) && parent->startWith) {
            /*
...
static MprHash *makeTokens(cchar *path, MprHash *other)
{
    ...
    tokens = mprDeserialize(sfmt(
                                ...
                                app->name, app->title, route->home, route->documents, httpGetDir(route, "CONTENTS"),
                                app->binDir, app->database,
                                filename, list, app->listen, app->controller, stitle(app->controller), app->controller,
                                stitle(app->controller),
                                app->table));
    if (other) {
